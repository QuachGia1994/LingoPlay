...
## Falsifiable USP
For supported devices, LingoPlay processes the video/audio media path locally and does not upload the user's video or audio to the backend; only compact translation JSON and store transaction/purchase identifiers needed for Plus verification cross the LingoPlay network boundary.
...
Stage 21 uses reserved weekly/monthly IDs `com.lingoplay.plus.weekly` and `com.lingoplay.plus.monthly` on both stores. Production Plus is server-authoritative: StoreKit verification or Google Play `PURCHASED` is only the client/store input; Release unlocks only after the LingoPlay Worker verifies the transaction/token with the corresponding store API. Until developer accounts exist, `Products.storekit` remains a DEBUG-only Xcode test catalog and Android honestly reports Play products unavailable.
...
- No client-embedded translation or store-provider verification secrets.
...
The consumer flow should be testable end-to-end as: pick video → explicit Speech AI install if needed → local ASR → transcript-only translation → local Vietnamese TTS → mix/remux → Library/playback/export. The optional neural path adds a separate explicit 64 MiB download (~78 MiB installed), checksum validation, user selection, and system-voice fallback. Paid capability gating remains fail-closed. Stage 21 implements the account-independent server verification boundary now; real App Store/Play products, provider credentials, signed store builds, notifications, refund/revocation and sandbox/license-tester evidence remain external blockers listed in `docs/release/store-readiness.md`.
