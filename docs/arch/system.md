...
## Current stage
Real local media ingestion, audio preparation, ASR, and transcript-only translation boundaries are wired on both native clients. Demo library/player content still exists only for visual navigation. The primary Import → Prepare → Processing path now requires an actual local video, displays real metadata, prepares audio locally, runs a genuinely installed on-device speech model, and then translates timestamped transcript batches when a backend endpoint/provider is configured. Missing ASR models, missing translation endpoint configuration, provider failures, and invalid provider responses all stop explicitly without fabricated output. TTS, duration fitting, source separation, mixing, and final remux remain pending.
