...
- Stage 15.3 fixes the real-device 40% translation stop by wiring the public translation endpoint into both Android and iOS CI artifacts. Both workflows now fail closed when `LINGOPLAY_TRANSLATION_API_BASE_URL` is missing instead of shipping an artifact that reaches ASR and then reports `translation not configured`.
- The Cloudflare backend now binds Workers AI and translates transcript-only segment JSON with pinned `@cf/meta/m2m100-1.2b`; video/audio remain device-local. Backend regression coverage is 10 tests, and the legacy external-provider proxy remains available as an optional fallback.
